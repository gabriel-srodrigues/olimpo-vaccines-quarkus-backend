...
@Slf4j
@ApplicationScoped
public class PatientVaccinatedNotifyStrategy implements NotifyStrategy {
    @Override
    public void send(SendNotificationUseCase.SendNotificationCommand command) {
        log.info("[PatientVaccinatedNotifyStrategy]: Sending patient vaccinated notification: {}", command);
    }
}
